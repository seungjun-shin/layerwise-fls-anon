...
Compares the after-late boundary against exact LR-only at the first checkpoint
where each arm crosses the same held-out validation-accuracy target.
...
BOUNDARY = "val_bnd_cut8_iso"
LRONLY = "val_lr_cut8_iso"
SEEDS = [0, 1, 2, 3, 4]
TARGETS = [55.0]
OUTDIR = Path("outputs/depth_profiles_isoacc_val")
PLOT_TABLE = Path("paper/tables/iso_accuracy_resnet_plot.csv")
...
def main() -> None:
    ...
    for t in TARGETS:
        ...
        for b in sorted(set(bb) & set(lb)):
            ...
            if any(result[3] != len(SEEDS) for result in (dd, de, da)):
                raise RuntimeError(f"Incomplete five-seed ResNet diagnostics at block {b}")
    ...
    if not plot_rows:
        raise RuntimeError("No complete ResNet iso-accuracy rows were produced")
